...
//  MirrorflyUIkit
//
//  Created by John on 29/07/22.
//

import Foundation
import AVFoundation

protocol AppAudioRecorderDelegate {
...
let audioRecordingMximumDuration : CGFloat = 300.0

class AppAudioRecorder: NSObject {
    
    //MARK:- Instance
    static let shared = AppAudioRecorder()
    
    var appAudioRecorderDelegate : AppAudioRecorderDelegate?
    ...
    private func InitialSetup(){
        ...
        do{ /// Setup audio player
            try audioSession.setCategory(AVAudioSession.Category.playAndRecord, options: [.defaultToSpeaker, .allowBluetooth, .allowAirPlay])
        ...
        if let audioRecorder = audioRecorder{
            if !isRecording {
                do{
                    ...
                    debugLog("Recording")
                    appAudioRecorderDelegate?.didStartRecording(fileName: fileName ?? "")
                } catch let recordingError as NSError{
                    print ("Error recording : %@", recordingError.localizedDescription)
                    appAudioRecorderDelegate?.didErrorOccur()
                }
            }
        }
    }
    ...
    @objc func updateDuration() {
        if isRecording && !isPlaying{
            duration += 1
            appAudioRecorderDelegate?.whileRunningTimer(duration: duration)
        }else{
            timer?.invalidate()
        }
    }
...
extension AppAudioRecorder : AVAudioRecorderDelegate{
    func audioRecorderDidFinishRecording(_ recorder: AVAudioRecorder, successfully flag: Bool) {
        ...
        switch flag {
            case true:
                ...
                appAudioRecorderDelegate?.didFinishRecording(url: recorder.url)
            case false:
    ...
    func audioRecorderBeginInterruption(_ recorder: AVAudioRecorder) {
        debugLog("AppAudioRecorder audioRecorderBeginInterruption \(isRecording)")
    }
    
    func audioRecorderEndInterruption(_ recorder: AVAudioRecorder, withOptions flags: Int) {
        debugLog("AppAudioRecorder audioRecorderEndInterruption \(isRecording)")
    }
}


//MARK:- AVAudioPlayer Delegate functions
extension AppAudioRecorder: AVAudioPlayerDelegate{
